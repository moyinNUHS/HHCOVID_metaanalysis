/**
 ...
 parameters {
   ...
   real<lower=0> sigmasq_b; //slope variance 
}
...
transformed parameters {
 ...
 p1_1 = inv_logit(a[1] + b[1]*hhfreq[1,1] + c*maskhrs[1,1] );
 p1_2 = inv_logit(a[1] + b[1]*hhfreq[1,2] + c*maskhrs[1,2] );
 p1_3 = inv_logit(a[1] + b[1]*hhfreq[1,3] + c*maskhrs[1,3] );
 
 pi1_1 = 1-(1-p1_1)^followupdays[1];
 pi1_2 = 1-(1-p1_2)^followupdays[1];
 pi1_3 = 1-(1-p1_3)^followupdays[1];
 
 p2_1 = inv_logit(a[2] + b[2]*hhfreq[2,1] + c*maskhrs[2,1] );
 p2_2 = inv_logit(a[2] + b[2]*hhfreq[2,2] + c*maskhrs[2,2] );
 p2_3 = inv_logit(a[2] + b[2]*hhfreq[2,3] + c*maskhrs[2,3] );
 
 pi2_1 = 1-(1-p2_1)^followupdays[2];
 pi2_2 = 1-(1-p2_2)^followupdays[2];
 pi2_3 = 1-(1-p2_3)^followupdays[2];
 
 p3_1 = exp(a[3] + b[3]*hhfreq[3,1] + c*maskhrs[3,1] );
 p3_2 = exp(a[3] + b[3]*hhfreq[3,2] + c*maskhrs[3,2] );
 p3_3 = exp(a[3] + b[3]*hhfreq[3,3] + c*maskhrs[3,3] );
 
 p4_1 = exp(a[4] + b[4]*hhfreq[4,1] + c*maskhrs[4,1] );
 p4_2 = exp(a[4] + b[4]*hhfreq[4,2] + c*maskhrs[4,2] );
 
 p5_1 = inv_logit(a[5] + b[5]*hhfreq[5,1] + c*maskhrs[5,1] );
 p5_2 = inv_logit(a[5] + b[5]*hhfreq[5,2] + c*maskhrs[5,2] );
 p5_3 = inv_logit(a[5] + b[5]*hhfreq[5,3] + c*maskhrs[5,3] );
